import { Dimensions } from "react-native";

export const SPACING_XXS = 4;
export const SPACING_XS = 8;
export const SPACING_SM = 12;
export const SPACING_MD = 16;
export const SPACING_LG = 20;
export const SPACING_XL = 24;
export const SPACING_XXL = 32;

export const SAFE_AREA_MIN_BOTTOM = SPACING_SM;

export const TAB_BAR_HORIZONTAL_MARGIN = SPACING_MD;
export const TAB_BAR_BASE_HEIGHT = 56;
export const TAB_BAR_BOTTOM_INSET_MULTIPLIER = 0.25;

export const SCREEN_HEIGHT_0_1 = Dimensions.get("window").height * 0.1;
export const SCREEN_HEIGHT_0_3 = Dimensions.get("window").height * 0.3;
export const SCREEN_HEIGHT_0_5 = Dimensions.get("window").height * 0.5;
export const SCREEN_HEIGHT_0_7 = Dimensions.get("window").height * 0.7;
export const SCREEN_HEIGHT_0_9 = Dimensions.get("window").height * 0.9;
export const SCREEN_HEIGHT_1_0 = Dimensions.get("window").height;
