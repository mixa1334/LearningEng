...
export function useLearningDailySet() {
  ...
  const { reviewWord, wordsToReview, learnWord, wordsToLearn, error } =
    useAppSelector((s) => s.learn);
...
export function useLearnUtil() {
  ...
  const markWordReviewed = (word: Word) =>
    dispatch(markWordReviewedThunk(word));
  const markWordNotReviewed = () => dispatch(skipWordWhenReviewing());
  const startLearnNewWord = (word: Word) => dispatch(startLearnWordThunk(word));
  const markWordCompletelyLearned = (word: Word) =>
